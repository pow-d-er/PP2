#include "stos_unsigned_int.h"

stack_unsigned_int_t* stack_unsigned_int_create(int size){
stack_unsigned_int_t* new = malloc(sizeof(stack_unsigned_int_t));
new->size = size;
new->howMuch = 0;
new->prevStack = NULL;
new->value = 0;
...
void stack_unsigned_int_push(stack_unsigned_int_t* pstack, unsigned int value){
  ...
  stack_unsigned_int_t* next = malloc(sizeof(stack_unsigned_int_t));
  next->prevStack = malloc(sizeof(stack_unsigned_int_t));
  next->value = value;
...
bool stack_unsigned_int_try_push(stack_unsigned_int_t* pstack, unsigned int value){
    if(pstack->howMuch == pstack->size){
      ...
      stack_unsigned_int_push(pstack, value);
      return true;
    }
...
int stack_unsigned_int_pop(stack_unsigned_int_t* pstack){
  unsigned int value = pstack->value;
...
void stack_unsigned_int_free(stack_unsigned_int_t* pstack){
  stack_unsigned_int_t* current_stack = malloc(sizeof(stack_unsigned_int_t));
...
void stack_unsigned_int_print(const stack_unsigned_int_t* pstack){
  if(stack_unsigned_int_empty(pstack)) printf("Stos jest pusty\n");
  else{
  printf("\nZawartość stosu: \n");
  stack_unsigned_int_t *stack = malloc(sizeof(stack_unsigned_int_t));
...
void save_stack_unsigned_int(const char* path, stack_unsigned_int_t* pstack){
  ...
  if(fp){
    ...
    fwrite(pstack,sizeof(stack_unsigned_int_t),1,fp);
...
stack_unsigned_int_t* loaded_stack = malloc(sizeof(stack_unsigned_int_t));
  if(fp){
  fread(loaded_stack,sizeof(stack_unsigned_int_t),1,fp);
  }else printf("Błąd odczytu pliku");
  fclose(fp);
